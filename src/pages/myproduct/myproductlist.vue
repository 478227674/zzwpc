      ...
      <div class="mianbox">
        ...
        <div style="background: #fff;">
          <ul class="classes-table">
            <li v-for="item in videoList" @click="openMyProductListBySortId(item)">
...
//    this.$emit('footer',false)
  ...
  methods:{
    ...
    //根据项目ID获取课程列表
    getTestListFn(){
      this.http.post('/dir/queryOrderListByAlClassId',{sortId:this.sortId,pageNum:this.pagenum,pageSize:10}).then(res=>{
        if(res.code == 0){
    ...
    //获取学科列表
    getSortList(){
      this.http.post('/dir/querySortIdByOrder',{}).then(res=>{
        if(res.code == 0){
          this.sortList = res.data;
          this.sortId = res.data[0].id;
          this.getTestListBySortId(this.sortId)
        }
      })
